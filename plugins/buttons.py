...
async def buttons(client:Client, callback_query:types.CallbackQuery):
    ...
    if relate == "lyrics":
        ...
        await client.message.reply_photo(self=client, photo=artist['picture_big'], caption=artist['name'], reply_markup=markup)
        ...
        markup = types.InlineKeyboardMarkup(keyboard)
        await client.message.reply_photo(self=client, photo=album['cover_big'], caption=F"{album['artist']['name']} - {album['title']}", reply_markup=markup)
        ...
        if downloading:
            ...
            for track in tracks:
                ...
                await client.send_audio(chat_id=query.message.chat.id, audio=pathlib.Path(download.song_path).read_bytes(), title=track['title'], performer=track['artist']['name'], duration=track['duration'])
