...
async def search(client:pyrogram.Client, message:pyrogram.types.Message):
    ...
    if "spotify" in site:
        if relate not in ("artist", "album", "track"):
            ...
            return await message.reply_text(text="Invalid Deezer URL!", reply_markup=markup)
        elif relate == "artist":
            ...
            markup = pyrogram.types.InlineKeyboardMarkup(keyboard)
            return await message.reply_photo(photo=artist["images"][0]["url"], caption=artist["name"], reply_markup=markup)
        elif relate == "album":
            ...
            markup = pyrogram.types.InlineKeyboardMarkup(keyboard)
            return await message.reply_photo(photo=track['album']['images'][0]['url'], caption=F"{track['artists'][0]['name']} - {track['name']}", reply_markup=markup)
    elif "spotify" not in site:
        return await message.reply_text(text=F"Invalid Spotify URL!\n\n1. The url must be from spotify.com\n2. If your given url is a playlist:\nThis Bot will not download playlists, Due to the playlist might be a day long")
    keyboard = [
